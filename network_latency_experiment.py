...
def run_experiment(master_inst_type, mach_array, data_set_name, data_bucket_name, runs, run_dependency, epochs, staleness_array, projected, chunks_to_use, init_time_str, ignore, full_ram_exp):
    ...
    for inst_type in inst_types_filter:
        ...
        for i in mach_array:
            ...
            num_chunks, num_digits, remainder, chunk_parts = distribute_chunks(mach_name_array, data_bucket_name, data_set_name, projected, chunks_to_use, full_ram_exp)
...
def main():
    ...
    #run_experiment('m3.2xlarge', [1,2,4,8,16], glob.DATA_SET, glob.DATA_SET_BUCKET, 30, 'independent', 10, [0], 1, 16, '', [])
    run_experiment('m3.2xlarge', [2,4], glob.DATA_SET, glob.DATA_SET_BUCKET, 30, 'independent', 10, [0], 0, 1, '-1_chunk_run', [], 1)
