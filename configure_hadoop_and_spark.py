...
def spark_config(nodes_info, up):
...
def start_spark(nodes_info, up):
    py_ssh('', nodes_info[0][0], 'spark-2.0.0/sbin/start-all.sh')

def stop_spark(nodes_info, up):
    py_ssh('', nodes_info[0][0], 'spark-2.0.0/sbin/stop-all.sh')

def scala_send_spark_job(nodes_info, up):
    for node in nodes_info:
        for jar in local_jar_path:
            print jar
            py_scp_to_remote('', node[0], jar, 'jars/' + jar.split('/')[-1])

def scala_run_spark_job(nodes_info, master_port, file_name, hadoop_master_port, jar):

    run_str = ['/home/ubuntu/spark-2.0.0/bin/spark-submit --verbose --master yarn' + 
               ' --deploy-mode cluster ' + ' file:///home/ubuntu/jars/'+ jar + ' yarn-cluster '
