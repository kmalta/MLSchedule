    ...
    for entry in filtered_dict_entries:
        ...
        for i in range(len(coordinates)):
            ...
            for j in range(diff_mach_nums):
                ...
                try:
                    rect_list.append(ax_list[i].bar([2*(k+.25) + j*width for k in range(len(matrix))], matrix_t[j], width, color=color_array[j]))
                except:
                    continue
        ...
        for j in range(diff_mach_nums):
            real_rects_list.append(ax5.bar([k+.75 + j*width/4 for k in range(len(fill_in_matrix))], [transform_func(scal, entry, dataset_name) for scal in matrix_t[j]], width/4, color=color_array[j]))
        ...
        for j in range(diff_mach_nums):
            ...
            ratio_rect_list.append(ax6.bar([k+.75 + j*width/4 for k in range(len(ram_ratios))], ram_ratios_t[j], width/4, color=color_array[j]))
...
def filter_coordinates_to_match_exps(coordinates):
    new_coords = ['1_0',
                  '2_0',
                  '3_0',
                  '4_-1',
                  ...
                  '7_-4',
                  '8_-5']
    coord_prefs = list(set(map(lambda x: x[0], coordinates)))
    coords = []
    for coord_pref in coord_prefs:
        for coord in new_coords:
            arr = coord.split('_')
            if str(coord_pref) == arr[0]:
                coords.append([int(arr[0]), int(arr[1])])
    return coords

def plot_communications(dataset_name, synth_data_dict, real_data_dict, coordinates, machine_num, data_set_num):
    color_array = ['blue', 'red', 'black', 'green', 'purple']
    width = 0.35
    ind = ['Comm', '.5gb', '1gb', '2gb']
    #ind = map(lambda x: x.split('_')[-1], synth_list)

    coordinates = filter_coordinates_to_match_exps(coordinates)
    ...
    for entry in filtered_dict_entries:
        ...
        for i in range(len(coordinates)):
            ...
            machine = synth_data_dict[coord_str]['worker_machine']
            ax_list[i].set_title(coord_str + " Computation Times")
            ...
            diffs = []
            for elem in matrix:
                diffs.append([x - y for x, y in zip(elem, matrix[0])])
            ...
            for j in range(diff_mach_nums):
                #ratios_t = map(list, zip(*ratios))
                diffs_t = map(list, zip(*diffs))
                try:
                    rects_list.append(ax_list[i].bar([2*(k+.25) + j*width for k in range(len(sizes_in_bytes))], diffs_t[j], width, color=color_array[j]))
                except:
                    continue
            ax_list[i].set_ylabel('Computation Time (s)')
            ax_list[i].set_xticks([2*(k+.25) + diff_mach_nums*width/2 for k in range(len(matrix))])
            labels = []
            for j in range(len(machine)):
                labels.append(ind[:len(diffs)][j] + ' ' + machine[j])
            ax_list[i].set_xticklabels(labels)
        ...
        ax6.set_ylabel('Ratios', fontsize=10)
        ax6.set_title('Dataset Size in GB(s) / ( RAM on Machine * # of Machines )', fontsize=12)
        ax6.set_xticks([k+.75 + diff_mach_nums*width/8 for k in range(len(ram_ratios))])
        ax6.set_xticklabels(machine_names)
...
def main():
    ...
    plot_communications(dataset, synth_data_dict, real_data_dict, synth_coordinates, 5, len(synth_list))
